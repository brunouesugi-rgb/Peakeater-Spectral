...
public:
    ...
    void reset() noexcept {
        ...
        recoveryCoefficient = releaseCoefficient;
    }
    ...
    Frame process(Frame input, bool limit = true) noexcept {
        ...
        recoveryGain = minimum < recoveryGain ? minimum
            : recoveryCoefficient * recoveryGain + (1.0f - recoveryCoefficient) * minimum;
        // Recover at most 0.5 dB of release loss, never above the same safe window.
        auto const scheduledGain = cycleAmount == 0.0f ? releaseGain
            : std::min(minimum, std::max(releaseGain, std::min(recoveryGain, releaseGain * recoveryCap)));
        ...
        gainSum += static_cast<double>(scheduledGain) - gains[gainWrite];
        gains[gainWrite] = scheduledGain;
...
private:
    ...
    void updateCycles(Frame const& input) noexcept {
        ...
        if ((clock & 15u) == 0) {
            ...
            auto const recoveryTime = std::max({8.0f, effectiveRelease * 0.25f, floor * confidence * cycleAmount});
            recoveryCoefficient = static_cast<float>(std::exp(-1.0 / (recoveryTime * 0.001 * sampleRate)));
            recoveryCap = 1.0f + 0.059253725f * (1.0f - lowShare) * (1.0f - lowShare) * cycleAmount;
        }
    }
    ...
    float recoveryGain = 1.0f, recoveryCoefficient = 0.0f, recoveryCap = 1.0f;
